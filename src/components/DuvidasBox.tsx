                    ...
                    <ButtonGroup>
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-44 bg-popover border-border text-popover-foreground">
                                <DropdownMenuGroup>
                                    <DropdownMenuItem className="cursor-pointer" onSelect={() => setFiltro("todas")}>
                                        Todos
                                    </DropdownMenuItem>
                                    <DropdownMenuItem className="cursor-pointer" onSelect={() => setFiltro("pendentes")}>
                                        <span className="text-red-600 dark:text-red-400 font-medium">Pendente</span>
                                    </DropdownMenuItem>
                                    <DropdownMenuItem className="cursor-pointer" onSelect={() => setFiltro("resolvidas")}>
                                        <span className="text-emerald-600 dark:text-emerald-400 font-medium">Resolvida</span>
                                    </DropdownMenuItem>
                                </DropdownMenuGroup>
                            </DropdownMenuContent>
                        </DropdownMenu>
            ...
            <div className="space-y-4">
                {duvidasFiltradasPorStatus.length > 0 ? (
                    duvidasFiltradasPorStatus.map((duvida) => {
                        ...
                        return (
                            <Card key={duvida.id} className="p-5 shadow-sm border border-border bg-card hover:shadow-md transition-shadow duration-200 space-y-4">
                                {/* Header da Dúvida */}
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="flex flex-col">
                                            <div className="flex items-center gap-2 flex-wrap mb-1">
                                                ...
                                                {duvida.resolvido ? (
                                                    <Badge className="bg-emerald-500 text-white hover:bg-emerald-600 dark:bg-emerald-600 dark:hover:bg-emerald-700 text-[10px] py-0 px-2 h-5 border-none">Resolvida</Badge>
                                                ) : (
                                                    <Badge variant="secondary" className="bg-red-500/10 text-red-600 border border-red-500/20 dark:text-red-400 dark:border-red-400/20 text-[10px] py-0 px-2 h-5">Pendente</Badge>
                                                )}
                                            </div>
            ...
            <Dialog open={modalAberto} onOpenChange={setModalAberto}>
                <DialogContent className="sm:max-w-[500px] bg-background border-border">
                    ...
                    <DialogFooter className="gap-2">
                        <Button variant="outline" className="cursor-pointer" onClick={() => setModalAberto(false)} disabled={enviandoResposta}>
                            Cancelar
                        </Button>
                        <Button
