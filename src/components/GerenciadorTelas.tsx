...
export function GerenciadorTelas(props: GerenciadorTelasProps) {
  ...
  const isNovoUsuario =
    !props.loadingInscricoes && props.usuario.listaDosInscritos.length === 0;
    ...
    <>
      {(props.usuario.acessouOq === "principal" ||
        ...
        !isMaster && (
          <BoasVindas
            perfil={props.perfil}
            acionarExplorar={() => props.navegarPara("pesquisar")}
          />
        )}
      {(props.usuario.acessouOq === "principal" ||
        props.usuario.acessouOq === "pesquisar") && (
        <div className="display grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {props.listaEscolar.turmas &&
            Object.entries(props.listaEscolar.turmas)
              .filter(
                ([key]) =>
                  props.estaInscrito(key) || props.perfil.role === "master",
              )
