...
export function Pesquisar(props: PesquisarProps) {
  ...
  const turmasBase =
    textoPesquisa.trim() === ""
      ? Object.entries(props.turmas).sort((a, b) =>
          a[1].materia.localeCompare(b[1].materia),
        )
      : turmasFiltradas;
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4">
      <Sheet open={aberto} onOpenChange={mudarAberturaSheet}>
        <SheetContent
          side="right"
          className="w-full sm:w-[400px] lg:w-[450px] flex flex-col p-5"
        >
            ...
            {turmasOrdenadas.length > 0 ? (
              <>
                <div className="text-xs text-muted-foreground mb-2">
                  {turmasOrdenadas.length} resultado
                  {turmasOrdenadas.length !== 1 ? "s" : ""}
                </div>
      ...
      >
        <DialogContent className="mensagens-dialog-exclusao sm:max-w-md">
          <DialogHeader className="items-center text-center sm:text-center">
            ...
            <DialogDescription>
              Você será inscrito em {turmaParaInscrever?.materia} (
              {turmaParaInscrever?.turma}) e terá acesso ao mural e aos
              conteúdos da turma.
            </DialogDescription>
          </DialogHeader>
