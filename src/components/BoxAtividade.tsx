...
type BoxAtividadeProps = {
    ...
    aberto: boolean
    onClose: () => void
    onPublicar: (titulo: string, descricao: string, dataEntrega: string | null, arquivos: File[]) => Promise<void>
    atividadeParaEditar?: Atividade | null
    onEditar?: (atividadeId: string, titulo: string, descricao: string, dataEntrega: string | null, arquivos: File[], urlsMantidas: string[]) => Promise<void>
}
...
export function BoxAtividade({ materia, aberto, onClose, onPublicar, atividadeParaEditar, onEditar }: BoxAtividadeProps) {
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            if (atividadeParaEditar && onEditar) {
                await onEditar(atividadeParaEditar.id, titulo, descricao, dataEntrega || null, arquivos, urlsMantidas)
            } else {
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-background/80 backdrop-blur-sm px-4 animate-in fade-in duration-200">
            <Card className="relative w-full max-w-xl p-6 shadow-2xl border border-border bg-card text-card-foreground max-h-[90vh] overflow-y-auto">
                ...
                <h2 className="text-xl font-bold tracking-tight mb-4">
                    {atividadeParaEditar ? `Editar Atividade em ${materia}` : `Nova Atividade para ${materia}`}
                </h2>
                ...
                <div className="flex justify-end gap-3 mt-8">
                    ...
                    <Button className="cursor-pointer" onClick={handleConfirm} disabled={!titulo.trim() || !descricao.trim() || loading}>
                        {loading ? (
                            ...
                            atividadeParaEditar ? "Salvar Alterações" : "Publicar Atividade"
                        )}
                    </Button>
                </div>
            </Card>
