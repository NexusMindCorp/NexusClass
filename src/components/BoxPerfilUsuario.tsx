...
export function BoxPerfilUsuario({
  nomeUsuario,
  onClose,
  currentUserProfile,
}: BoxPerfilUsuarioProps) {
  const { perfilAlvo, loading, error, ehProprioUsuario } = usePerfilUsuario({
    nomeUsuario,
    currentUserProfile,
  });
          ...
          {loading ? (
            <div className="py-12 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-sm text-muted-foreground">
                Buscando perfil no banco de dados...
              </p>
            </div>
          ) : error ? (
            ...
            <div className="w-full flex flex-col items-center">
              ...
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-primary/10 text-primary mb-4">
                ...
                {ehProprioUsuario
                  ? `Seu Perfil (${perfilAlvo.role})`
                  : `Perfil de ${perfilAlvo.role}`}
              </span>
              ...
              <div className="w-full mt-1 pt-2 flex justify-center gap-1">
                <span className="text-xs text-muted-foreground flex items-center gap-1.5">
                  ...
                  Membro desde{" "}
                  {perfilAlvo.created_at
                    ? new Date(perfilAlvo.created_at).toLocaleDateString(
                        "pt-BR",
                        {
                          day: "numeric",
                          month: "long",
