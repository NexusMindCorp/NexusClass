  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
            ...
            {messages.map((msg, idx) => {
              ...
              return (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`w-fit p-3 rounded-2xl text-[13px] leading-relaxed whitespace-pre-wrap break-words shadow-sm ${msg.role === 'user'
          ...
          <div className="p-4 bg-gray border-t border-gray-100">
            <div className="relative flex items-center">
              ...
              <button
                ...
                className="absolute right-2 p-2 cursor-pointer text-indigo-600 hover:text-indigo-800 disabled:cursor-not-allowed disabled:text-gray-300 transition-colors"
                type="button"
              >
                <Send size={18} />
              </button>
      ...
      <button
        ...
        className={`w-14 h-14 rounded-full flex cursor-pointer items-center justify-center text-white shadow-2xl transition-all duration-300 hover:scale-110 active:scale-95 ${isOpen ? 'bg-gray-900 rotate-180' : 'bg-indigo-600'
          }`}
      >
