...
export function BoxAgendamento({
  cancelaAgendamento,
  usaSupabase,
  date,
  titulo,
  ...
  const textoDataEvento = date
    ? format(date, "dd/MM/yyyy")
    : "data nao selecionada";
  const [horaInicial = "", minutoInicial = ""] = horario.split(":");
  const horas = Array.from({ length: 24 }, (_, indice) =>
    String(indice).padStart(2, "0"),
  );
  const minutos = Array.from({ length: 60 }, (_, indice) =>
    String(indice).padStart(2, "0"),
  );
  ...
  return (
    <div className="min-w-0 space-y-4 rounded-xl border border-border bg-card p-3 shadow-lg shadow-black/5 sm:p-4">
      ...
      <div className="space-y-1.5">
        <Label htmlFor="titulo-evento" className="text-foreground">
          Título do evento
        </Label>
        <Input
          id="titulo-evento"
      ...
      <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="flex h-10 min-w-0 flex-1 items-center gap-1 rounded-lg bg-muted px-1.5 sm:gap-2 sm:px-2">
              ...
              >
                ...
                <SelectContent
                  ...
                  collisionPadding={16}
                  avoidCollisions
                  className="w-[4.5rem] max-h-56 overflow-hidden rounded-xl border border-border bg-popover/95 text-popover-foreground shadow-xl backdrop-blur"
                >
                  {horas.map((hora) => (
                    <SelectItem
                      key={hora}
        ...
        <div className="space-y-1.5 sm:col-span-2">
          <Label htmlFor="descricao-evento" className="text-foreground">
            Descrição (opcional)
          </Label>
          <Textarea
            id="descricao-evento"
